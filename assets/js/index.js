...
window.addEventListener("load", function () {
  ...
  this.document.querySelectorAll(".nav--link").forEach((navLink) => {
    suffixToReplace = navLink.href.substring(
      navLink.href.length - 3,
      navLink.href.length,
    );
  ...
  if (myProjectsLink != null) {
    suffixToReplace = myProjectsLink.href.substring(
      myProjectsLink.href.length - 3,
      myProjectsLink.href.length,
    );
    myProjectsLink.href = myProjectsLink.href.replace(suffixToReplace, lang);
  }

  // language select buttons
  this.document.querySelector(`#LSel${lang}`).className =
    "langbtns langbtns--hlghtd";
